...
{
    $environmentKey = getenv('GROQ_API_KEY') ?: ($_ENV['GROQ_API_KEY'] ?? '');
    if (is_string($environmentKey) && trim($environmentKey) !== '') {
        return trim($environmentKey);
    }

    // Fichier local non versionné, pratique pour une installation XAMPP.
    $localFile = __DIR__ . '/groq.local.php';
    if (is_file($localFile)) {
        $local = require $localFile;
        if (is_array($local) && !empty($local['api_key'])) {
            return trim((string) $local['api_key']);
        }
    }
    return '';
}
